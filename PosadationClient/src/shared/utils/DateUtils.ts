...
export class DateUtils {
  public static serializeToString(date: Date): string {
    return `${this.formatWith0(date.getFullYear(), 4)}-${this.formatWith0(date.getMonth() + 1, 2)}-${this.formatWith0(date.getDate(), 2)}`
  ...
  public static prettyPrintDate(date: Date, locale: string): string {
    ...
    if (locale === 'es-mx') {
      return `${this.formatWith0(date.getDate(), 2)}/${this.formatWith0(date.getMonth() + 1, 2)}/${this.formatWith0(date.getFullYear(), 4)}`
    }

    return `${this.formatWith0(date.getMonth() + 1, 2)}/${this.formatWith0(date.getDate(), 2)}/${this.formatWith0(date.getFullYear(), 4)}`
  }
